jQuery(document).ready(function($) {
    if ($('.cd-form').length > 0) {
        ...
        //show/hide mpesa fields if user selected mpesa as gateway
        $('.cd-payment-gateways').on('change', function() {
            ($('#mpesa').is(':checked')) ?
            $('.cd-mpesa').velocity("slideDown", { duration: 300 }): $('.cd-mpesa').velocity("slideUp", { duration: 300 });
        });

        //show/hide credit card fields if user selected credit card as gateway
        $('.cd-payment-gateways').on('change', function() {
            ($('#card').is(':checked')) ?
            $('.cd-credit-card').velocity("slideDown", { duration: 300 }): $('.cd-credit-card').velocity("slideUp", { duration: 300 });
        });
